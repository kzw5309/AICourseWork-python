...
class Iff(Expr):
    ...
    def __eq__(self, other):
        return type(self) == type(other) and (self.left == other.right and self.right == other.left) or (
        ...
        while 1:
            ...
            for i in xrange(len(clauses_list)):
                for j in xrange(i + 1, len(clauses_list)):
                    # print new_facts_list[i]
                    # print new_facts_list[j]
                    ...
                    if temp_tuple not in pairs:
                        ...
                        if isinstance(clauses_list[i], Or):
                            if isinstance(clauses_list[j], Or):
                                # inter_b = clauses_list[i].disjuncts.intersection(clauses_list[j].disjuncts)
                                # if len(inter_b) == 0:
                                #     names_i = {x.atom_names().pop() for x in clauses_list[i].disjuncts}
                                #     names_j = {x.atom_names().pop() for x in clauses_list[j].disjuncts}
                                #     # inter_a = clauses_list[i].atom_names().intersection(clauses_list[j].atom_names())
                                #     inter_a = names_i.intersection(names_j)
                                #     if len(inter_a) == 1:
                                #         at = next(iter(inter_a))
                                #         if Atom(at) in clauses_list[i].disjuncts:
                                #             difference_i = clauses_list[i].disjuncts.difference({Atom(at)})
                                #             difference_j = clauses_list[j].disjuncts.difference({Not(Atom(at))})
                                #         else:
                                #             difference_i = clauses_list[i].disjuncts.difference({Not(Atom(at))})
                                #             difference_j = clauses_list[j].disjuncts.difference({Atom(at)})
                                #         resolvent = difference_i.union(difference_j)
                                not_i_d = {Not(x).to_cnf() for x in clauses_list[i].disjuncts}
                                inter_d = clauses_list[j].disjuncts.intersection(not_i_d)
                                if len(inter_d) == 1:
                                    different_j_d = clauses_list[j].disjuncts.difference(inter_d)
                                    not_inter_d = {Not(x).to_cnf() for x in inter_d}
                                    different_i_d = clauses_list[i].disjuncts.difference(not_inter_d)
                                    resolvent = different_i_d.union(different_j_d)
                            else:
                                not_j = Not(clauses_list[j]).to_cnf()
                                not_j_set = {not_j}
                                if not_j in clauses_list[i].disjuncts:
                                    resolvent = clauses_list[i].disjuncts.difference(not_j_set)
                        else:
                            not_i = Not(clauses_list[i]).to_cnf()
                            not_i_set = {not_i}
                            if isinstance(clauses_list[j], Or):
                                if not_i in clauses_list[j].disjuncts:
                                    resolvent = clauses_list[j].disjuncts.difference(not_i_set)
                            else:
                                if not_i == clauses_list[j]:
                                    return True
                        if resolvent:
                            if len(resolvent) == 0:
                                return True
...
#kb3.tell(And(p1, e2, Not(p2), Not(e1)))
...
puzzle_4_question = """
The guilty man is Brown who also lies. Adams and Clark are innocent.
"""
...
feedback_question_2 = """
The most challenging part of this assignment is design a efficient ask function.
"""
